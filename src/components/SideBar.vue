<template>
  <v-card min-width="320">
    <v-img :src="user.avatar" height="300px" class="px-3 py-3" dark ref="img">
      ...
      <v-btn dark icon @click="handleClick" color="blue lighten-3" v-if="user.id === loginUser.id">
...
export default {
  data: () => ({
    ...
    selectedItem: 0,
    items: [
      { text: '资料编辑', icon: 'mdi-folder', path: '/layout/my/userinfo' },
      { text: '账户安全', icon: 'mdi-account-multiple', path: '/layout/my/userinfo/usersafe' },
      { text: '意见反馈', icon: 'mdi-star', path: '/layout/my/userinfo/feekback' },
      { text: '关于社区', icon: 'mdi-history', path: '/layout/my/about' },
    ],
    genders: ['保密', '男', '女'],
  }),
  filters: {
