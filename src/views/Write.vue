<template>
  <v-main class="container mt-8" >
    ...
    <v-row class="mx-4 mb-3">
      <!-- 遍历标签，给每个标签一个复选框，绑定value的值为标签的名称 -->
      <v-checkbox
...
export default {
  ...
  methods: {
    ...
    submit() {
      // 标签数组
      let list = []
      //遍历复选框数组，得到选中的标签，和一个空的文章id字符串拼成ArticleTag对象，加入list数组，到后端再补全随机文章id
      this.tagList.forEach((element) => {
        let articleTag = { articleId: '', tagName: element }
        list.push(articleTag)
      })
      //创建arcle对象，即向后端请求发文章接口需要传递的参数
      let article = {
      ...
      this.axios({
        ...
        //发表成功，跳回首页，可换成自己的弹框
        if (res.data.code === 1) {
          this.$layer.alert('发表成功')
          this.$router.push('/layout/index')
        } else {
          this.$layer.alert('发表失败')
        }
      })
    },
  },
